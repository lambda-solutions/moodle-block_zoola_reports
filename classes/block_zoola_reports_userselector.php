...
/**
 * Description of block_zoola_reports_userselector
 *
 * @author vukas
 */
class block_zoola_reports_userselector extends block_zoola_reports_selector_input {
     ...
     */
    public function block_zoola_reports_userselector($elementName = null, $elementLabel = null, $options = null, $attributes = null) {
        self::__construct($elementName, $elementLabel, $options, $attributes);
    }

    protected function initialize_selector($elementName = null, $options = null) {
        $this->_type = 'block_zoola_reports_userselector';
        if ($elementName) {
            $this->available_selector = new block_zoola_reports\potential_users($elementName . "_available", $options);
            $this->selected_selector = new block_zoola_reports\selected_users($elementName . "_selected", $options);
        }
        $this->itemsKey = BLOCK_ZOOLA_REPORTS_USERS_KEY;
    }

    protected function remove_from_list(array $elements) {
        global $SESSION;
        $sessionKey = BLOCK_ZOOLA_REPORTS_SESSION_KEY;
        $newelements = array_values(array_unique(array_diff($SESSION->{$sessionKey}[$this->blockinstanceid]->{$this->itemsKey}, array_keys($elements))));
    ...
    protected function add_to_list(array $elements) {
        global $SESSION;
        $sessionKey = BLOCK_ZOOLA_REPORTS_SESSION_KEY;
        $newelements = array_values(array_unique(array_merge($SESSION->{$sessionKey}[$this->blockinstanceid]->{$this->itemsKey}, array_keys($elements))));
        $SESSION->{$sessionKey}[$this->blockinstanceid]->{$this->itemsKey} = $newelements;
    }
